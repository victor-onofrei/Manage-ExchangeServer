...
foreach ($exchangeObject in $params.exchangeObjects) {
    ...
    if ($itemCount -le $itemCountThreshold -or $BypassItemCount) {
        ...
        if (!$hasArchive) {
            ...
            [System.String]$message = "Mailbox $exchangeObject has on-premise archive enabled. " +
                "Processing the script requires permanently disabling the archive which will " +
                "result in data loss. Consider merging the 2 mailbox objects manually after " +
